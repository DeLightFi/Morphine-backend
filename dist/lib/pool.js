...
class PoolValuesFetcher {
    constructor() {
        ...
        this.compiledABI = starknet_2.json.parse(fs.readFileSync(require.resolve("./abi/pool.json")).toString("ascii"));
    }
    async CallContract(pooladdress) {
        const poolContract = new starknet_2.Contract(this.compiledABI.abi, pooladdress, this.provider);
